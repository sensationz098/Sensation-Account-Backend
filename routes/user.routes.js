...
router.post("/student/addbatch", authenticateUser, async (req, res) => {
  ...
  try {
    ...
    await newbatch.save();
    res.status(201).json({
      message: "NewBatch added successfully",
      ok: true,
    });
  } catch (err) {
...
router.get("/student/batches", authenticateUser, async (req, res) => {
  try {
    const batches = await BatchModel.find(); // fetch all batches
...
router.put(
  "/student/extend-course/:studentId",
  authenticateUser,
  async (req, res) => {
    try {
      // Extract user ID from the authenticated user
      ...
      if (
        ...
        isNaN(additionalMonths) ||
        additionalMonths < 0
      ) {
        return res
          .status(400)
          .json({ message: "Invalid or missing additional course duration" });
      }
      ...
      if (!date_of_payment || isNaN(Date.parse(date_of_payment))) {
        return res
          .status(400)
          .json({ message: "Invalid or missing date_of_payment" });
      }
      ...
      if (additionalMonths > 0) {
        student.CourseDuration =
          parseInt(student.CourseDuration) + additionalMonths;
  ...
  try {
    ...
    students.forEach((student) => {
      student.enrolledCourses.forEach((course) => {
        const isCourseExtended =
          student.previousCourses && student.previousCourses.length >= 1;
        const data = {
          name: student.name,
          contact: student.contact,
          email: student.email,
...
router.get("/students/filter", authenticateUser, async (req, res) => {
  try {
    ...
    if (courseStartDate) {
      filterCriteria["enrolledCourses.courseStartDate"] = new Date(
        courseStartDate
      );
    }
...
router.get("/display-and-download", authenticateUser, async (req, res) => {
  try {
    ...
    const validStudents = students.filter((student) => {
      return (
        student.courseEndDate instanceof Date &&
        !isNaN(student.courseEndDate.valueOf())
      );
    });
          ...
          student.previousCourses.forEach((previousCourse) => {
            ...
            if (student.previousCourses && student.previousCourses.length > 0) {
              student.previousCourses.forEach((prevCourse, index) => {
                data[`prevCourseEndDate${index + 1}`] = formatDate(
                  new Date(prevCourse.end)
                );
              });
            }
...
const setFilterCriteria = (req) => {
  ...
  if (!isEmpty(req.query.PaymentDate)) {
    ...
    if (!isNaN(parsedDate.valueOf())) {
      ...
      filterCriteria["$or"] = [
        ...
        {
          "previousCourses.date_of_payment": {
            $gte: startOfDay,
            $lt: endOfDay,
          },
        },
  ...
  if (!isEmpty(req.query.coursename)) {
    if (req.query.coursename.toLowerCase() === "null") {
      ...
      filterCriteria["course"] = {
        $regex: new RegExp(req.query.coursename, "i"),
      };
    }
  }

  if (
    !isNaN(req.query.fees) &&
    req.query.fees !== null &&
    req.query.fees !== ""
  ) {
  ...
  if (!isEmpty(req.query.teacher)) {
    if (req.query.teacher.toLowerCase() === "null") {
      ...
      filterCriteria["Teacher"] = {
        $regex: new RegExp(req.query.teacher, "i"),
      };
    }
  }
  ...
  if (!isEmpty(req.query.timing)) {
    if (req.query.timing.toLowerCase() === "null") {
      ...
      const escapedTiming = req.query.timing.replace(
        /[.*+?^${}()|[\]\\]/g,
        "\\$&"
      );
  ...
  Object.keys(filterCriteria).forEach(
    (key) => isEmpty(filterCriteria[key]) && delete filterCriteria[key]
  );
...
router.get("/students/testing", authenticateUser, async (req, res) => {
  try {
    ...
    if (students.length === 0) {
      ...
      const defaultStudents = await studentModel
        .find(defaultFilterCriteria)
        .lean();
...
router.get("/students/count", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "An error occurred while counting students." });
  }
});
